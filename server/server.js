...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    if (existing) {
      ...
      if (room && room.state === "playing") {
        ...
        console.log(
          `🏆 ${otherPlayer} ha vinto automaticamente perché ${socket.userId} si è disconnesso nella stanza ${roomId}`
        );
      }
    }
  });
  ...
  socket.on("playerReady", () => {
    ...
    room.nReady += 1;
    console.log(
      `⚡ ${socket.userId} è pronto nella stanza ${roomId} (${room.nReady}/2)`
    );
    sendRoomUpdate(roomId);
  ...
  socket.on("playerNotReady", () => {
    ...
    room.nReady -= 1;
    console.log(
      `⚡ ${socket.userId} non è più pronto nella stanza ${roomId} (${room.nReady}/2)`
    );
    sendRoomUpdate(roomId);
  });
  ...
  function startTurnTimer(roomId) {
    ...
    room.turnTimer = setTimeout(() => {
      ...
      console.log(
        `⏰ Turno di ${room.currentTurn} è scaduto nella stanza ${roomId}`
      );
  ...
  socket.on("cellClick", (cellIndex) => {
    ...
    if (room.playersData[socket.userId].score >= 5) {
      ...
      // le celle rimaste non devono più essere cliccabili (da gestire lato client)
  ...
  socket.on("leaveGame", () => {
    ...
    console.log(
      `🏆 ${otherPlayer} ha vinto automaticamente perché ${socket.userId} ha abbandonato la partita nella stanza ${roomId}`
    );
    leaveRoom(socket, roomId);
    socket.emit("roomLeft");
  });
